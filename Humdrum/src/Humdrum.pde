...
//array of cubes
int numOfCubes = 1 ; //number of balls (500)
...
void setup() {
  ...
  for (int i=0; i<numOfCubes; i++) {
    //bouncingBalls[i] = new FCircle(15);
    ...
    bouncingCubes[i].setStrokeWeight(0.5);
    bouncingCubes[i].setFill(255, 255, 255, 150); 
    bouncingCubes[i].setVelocity(0, 0);
    bouncingCubes[i].setRestitution(0.01);
    bouncingCubes[i].setDamping(0.1);
...
void draw() {
  ...
  if (start == false) {
    ...
    text("Why? (type your answer and press enter)", indent, 250);
...
void collectData(String file, String addText){
